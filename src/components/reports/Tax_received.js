  ...
  async function getTaxSum() {
    ...
    try {
      const response = await axios.get(
        `http://knowforth.online:8003/api/GetTaxSum/${compId}?startDate=${dateFrom}&endDate=${dateTill}`,
        {
          auth: {
            username: userName,
            password: passWord,
  ...
  const downloadPdf = () => {
    ...
    setTimeout(() => {
      ...
      html2canvas(input).then((canvas) => {
        ...
        pdf.save(`TaxSum_${currentyear}${currentMonth}${currenDate}${hour}${minute}.pdf`);
      });
                ...
                <div className="dropdown mx-2 d-flex justify-content-end">
                  ...
                  >
                    ...
                    <TableToExcel
                      ...
                      filename={`TaxSum_${currentyear}${currentMonth}${currenDate}${hour}${minute}`}
                      sheet="sheet 1"
